...
import uuid
from neo4j_database import GraphDatabase, Session, Transaction, basic_auth
from flask import g
...
def create_user(tx: Transaction, name: str, lastname: str, email: str, password: str, role: str, provider: str):
    return tx.run(
        """
        CREATE (user:User {id: $id, name: $name, lastname: $lastname, email: $email, password: $password, role: $role, rovider: $provider}) RETURN user
        """,
        {
            "id": str(uuid.uuid4()),
            "name": name,
